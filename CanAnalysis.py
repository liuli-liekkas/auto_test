...
if __name__ == '__main__':
    ...
    ret = canDLL.VCI_Transmit(Struct_USBCAN2C, 0, 0, ctypes.byref(struct_can_obj), 1)
    if ret != STATUS_OK:
    ...
    struct_can_obj = Struct_CAN_OBJ(0x0, 0, 0, 1, 0, 0, 8, a, b)
    ret = canDLL.Receive(Struct_USBCAN2C, 0, 1, ctypes.byref(struct_can_obj), 1, 0)
    print(ret)
    while ret <= 0:
        print('调用 Struct_Receive 出错\r\n')
        ret = canDLL.VCI_Receive(Struct_USBCAN2C, 0, 1, ctypes.byref(struct_can_obj), 1, 0)
    if ret > 0:
